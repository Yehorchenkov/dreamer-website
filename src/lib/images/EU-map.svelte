<script>
	...
	// Store a reference to any pending hide-timer
	let hideTooltipTimeout = null;
	const HIDE_DELAY = 500; // milliseconds

	let tooltipContent = $derived(hoveredCountry ? countryData[hoveredCountry] : null);
	let showTooltip = $derived(isTooltipHovered || hoveredCountry);

	// Helper to keep a value between min and max
	function clamp(value, min, max) {
    	return Math.min(Math.max(value, min), max);
  	}

	function handleMouseEnter(event, countryId) {
		...
		if(hoveredCountry != countryId) {
			...
			// Estimate (or measure) your tooltip dimensions:
			// You can tweak these or measure dynamically.
			const tooltipWidth = 200;
			const tooltipHeight = 80;
			const offset = 10; // distance from the cursor

			// The largest X/Y that keeps tooltip fully onscreen
			const maxX = window.innerWidth - tooltipWidth;
			const maxY = window.innerHeight - tooltipHeight;

			// Clamp the desired coords so the tooltip won't go offscreen
			newX = clamp(newX, offset, maxX - offset);
			newY = clamp(newY, offset, maxY - offset);
...
{#if tooltipContent && showTooltip}
	<div class="absolute z-10 bg-white p-2 rounded-lg shadow-lg"
		style="top: {tooltipY+10}px; left: {tooltipX-30}px;"
		role="tooltip"
		onmouseenter={handleTooltipMouseEnter}
		onmouseleave={handleTooltipMouseLeave}
	>
		<p>{tooltipContent.description}</p>
		<a href={tooltipContent.ref} target="_blank" rel="noopener noreferrer">Learn more</a>
	</div>
{/if}
...
<svg
	...
	width="100%"
	height="auto"
	viewBox="0 0 1189 1160.226"
	preserveAspectRatio="xMinYMid meet"
	xml:space="preserve"
	style="enable-background:new 0 0 1189 1160.226;"
	class="md:w-180 md:h-180"
>
<style type="text/css">
	.st0{
		fill:#166F75;
	...
	.st1{
		...
		transform-box: fill-box;     /* Ensures the bounding box is used for centering */
	}

	.st1:hover {
  		fill: #ffcc00; /*new color on hover */
...
<g>
	...
	<g class="st2">
		...
		<path class="st3" d="M586.6,536.1l3.4-6.4c2.4-1.1,10.5-8.3,12.2-4.6c1.4,2.9,3.4,14.3,1.1,16.5c-0.8,0.7-6.2,4.2-10.6,3.8    C588.4,545,585.8,538.8,586.6,536.1z"/>
		<path class="st3" d="M478.1,675.3c5.2-3.7,4.9,6.5,3.2,8.9c-2,2.1-5.1,6.1-8.1,3c-2.8-2.9,2-7.4,3.6-10.4    C477.1,676.2,477.6,675.7,478.1,675.3z"/>
